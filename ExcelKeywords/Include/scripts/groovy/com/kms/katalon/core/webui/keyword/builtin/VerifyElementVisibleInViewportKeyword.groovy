...
@Action(value = "verifyElementVisibleInViewport")
public class VerifyElementVisibleInViewportKeyword extends WebUIAbstractKeyword {
    ...
    public Object execute(Object ...params) {
        ...
        return verifyElementVisibleInViewport(to,timeOut,flowControl)
    }

    @CompileStatic
    public boolean verifyElementVisibleInViewport(TestObject to, int timeOut, FailureHandling flowControl) {
        WebUIKeywordMain.runKeyword({
            try {
                if (WebUiCommonHelper.isElementVisibleInViewport(DriverFactory.getWebDriver(), to, timeOut)) {
                    logger.logPassed(MessageFormat.format(StringConstants.KW_LOG_PASSED_OBJ_X_VISIBLE_IN_VIEWPORT, to.getObjectId()))
                    return true
                }  else {
